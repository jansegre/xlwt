...
class BitSet(object):
    ...
    def __init__(self,data=None):
        if not data:
            BitSet.__init__(self, [compat.long(0)])
            return
        if isinstance(data, six.integer_types):
            BitSet.__init__(self, [compat.long(data)])
            return
        if isinstance(data, six.integer_types):
        ...
        if not isinstance(data, list):
            raise TypeError("BitSet requires integer, long, or " +
                            "list argument")
        for x in data:
            if not isinstance(x,compat.long):
    ...
    def set(self,bit,on=True):
        ...
        if i>=len(self.data):
            d = i - len(self.data) + 1
            for x in xrange(0,d):
                self.data.append(compat.long(0))
