...
class XlsDoc:
    ...
    def __init__(self):
        ...
        self.dir_stream = six.b('')
        self.dir_stream_sect = []

        self.packed_SAT = six.b('')
        self.SAT_sect = []

        self.packed_MSAT_1st = six.b('')
        self.packed_MSAT_2nd = six.b('')
        self.MSAT_sect_2nd = []
    ...
    def __build_directory(self): # align on sector boundary
        ...
        dentry_name      = six.b('\x00').join(compat.iterbytes(six.b('Root Entry\x00'))) + six.b('\x00')
        dentry_name_sz   = len(dentry_name)
        dentry_name_pad  = six.b('\x00')*(64 - dentry_name_sz)
        ...
        dentry_name      = six.b('\x00').join(compat.iterbytes(six.b('Workbook\x00'))) + six.b('\x00')
        dentry_name_sz   = len(dentry_name)
        dentry_name_pad  = six.b('\x00')*(64 - dentry_name_sz)
    ...
    def __build_header(self):
        doc_magic             = six.b('\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1')
        file_uid              = six.b('\x00')*16
        rev_num               = six.b('\x3E\x00')
        ver_num               = six.b('\x03\x00')
        byte_order            = six.b('\xFE\xFF')
        log_sect_size         = struct.pack('<H', 9)
        log_short_sect_size   = struct.pack('<H', 6)
        not_used0             = six.b('\x00')*10
        total_sat_sectors     = struct.pack('<L', len(self.SAT_sect))
        dir_start_sid         = struct.pack('<l', self.dir_stream_sect[0])
        not_used1             = six.b('\x00')*4
        ...
        self.header = six.b('').join([doc_magic,
                                      file_uid,
                                      rev_num,
                                      ver_num,
                                      byte_order,
    ...
    def save(self, file_name_or_filelike_obj, stream):
        ...
        padding = six.b('\x00') * (0x1000 - (len(stream) % 0x1000))
        ...
        try:
            ...
            for offset in six.moves.xrange(0, len(stream), chunk_size):
